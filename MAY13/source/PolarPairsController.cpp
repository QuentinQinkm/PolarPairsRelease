...
void PolarPairsController::checkWinCondition() {
    if (_bearFinished && _penguinFinished && !_winConditionMet) {
        ...
        // Score +1 if both characters reached their finish blocks on the same move
        if (_bothFinishedSameMove) {
            score += 1;
        }
...
void PolarPairsController::updateFinishState() {
    ...
    _penguinFinished = false;
    bool penguinJustFinished = false;
    ...
    // Store flag if both characters reached finish blocks in the same move
    if (bearJustFinished && penguinJustFinished) {
        _bothFinishedSameMove = true;
    }
